...
class PersonEditCtrl {
	...
	public function validateSave() {
		...
		$this->form->phoneNumber = getFromRequest('phone',true,'Błędne wywołanie aplikacji');
		...
		if (empty(trim($this->form->phoneNumber))) {
		...
		if(!preg_match("/^\d{3}-\d{3}-\d{3}$/", trim($this->form->phoneNumber))){
			getMessages()->addError('Zły format numeru telefonu. Przykład 123-456-789');
		}
	...
	public function action_personEdit(){
		...
		if ( $this->validateEdit() ){
			try {
				...
				$this->form->phoneNumber = $record['phoneNumber'];
